...
def load_annotations(anno_obj, return_dict):
...
def load_predictions(prediction_obj, return_dict):
...
def compute_oks(anno, predict, delta):
    ...
    for i in range(anno_count):
        ...
        scale = np.float32((bbox[3]-bbox[1])*(bbox[2]-bbox[0])) # 面积
        if np.sum(visible) == 0:
            ...
            for j in range(predict_count):
                ...
                # delta是归一化因子, 可见主要受dis影响，说白了就是欧式距离，预测时，是不是visible对结果没有影响，只要标位置对了，就行
                print '====== keypoints score =========='
                print np.exp(-dis/2/delta[visible]**2/(scale+1))
...
def keypoint_eval(predictions, annotations, return_dict):
    ...
    for image_id in annotations['image_ids']:
        ...
        if image_id in prediction_id_set:
            ...
            #print 'warning: ' + image_id + ' is not in the prediction JSON file.'
    ...
    for threshold in np.linspace(0.5, 0.95, 10):
        # print oks_all, threshold, oks_num, np.sum(oks_all > threshold), np.sum(oks_all > threshold)/np.float32(oks_num)
        # oks_all, 0.05一个档，只要精度超过这个档就计1，如果精度超过0.95, 就是满分, 没掉0.05，就减一些分
        average_precision.append(np.sum(oks_all > threshold)/np.float32(oks_num))
    return_dict['score'] = np.mean(average_precision)

    return return_dict

def compare(annos, predicts):
    ...
    annotations = load_annotations(annos, return_dict=return_dict)

    # Load prediction JSON file
    start_time = time.time()
    predictions = load_predictions(predicts,
